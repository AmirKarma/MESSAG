...
func _on_Moonetengenerator_button_pressed():
	...
	if DataScript.getMoonstone() >= building_data[DataScript.MOONETEN_GENERATOR_CARD]["price"]:
		DataScript.set_inventory(DataScript.moonetenGenerator,"Moonetengenerator",DataScript.generators_upgrade_costs,"moonetenGenerator","res://Minigame2/Scenes/minigame2.tscn", 0, DataScript.generators_max_storage_size)
