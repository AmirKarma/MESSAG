## This script manages a shop interface in the game, allowing players to purchase various buildings using in-game resources.

extends Control

## Variable for the current building type
const building_type: int = 0  ## Represents the current type of building in the game.

var need_moonstone: bool = false  ## Indicates whether the building needs moonstone resource.
var need_mooneten: bool = false  ## Indicates whether the building needs mooneten resource.

## Variable for the building button
var currentButton: Button = null  ## Reference to the currently selected building button.

## Node reference for the building name label
...
## Node reference for the building count label
...
## Node reference for the building price label
...
## Node reference for the building image sprite
...
## Node reference for the price hsplit container
...
## Node reference for the building card
...
## Node reference for the mooneten amount label
...
## Node reference for the moonstone amount label
...
## Node reference for the buttons container
...
## Node reference for the buy button
...
## Node reference for the mooneten image sprite
...
## Node reference for the moonstone image sprite
...
## Node reference for the player character
...
## Called when the Mooneten Generator button is pressed. Checks if the player has enough Moonstone
...
func _on_Moonetengenerator_button_pressed():
	...
	if DataScript.get_moonstone() >= building_data[DataScript.MOONETEN_GENERATOR_CARD]["price"]:
		DataScript.set_inventory(
			...
			"moonetenGenerator",
			"res://Minigame2/Scenes/minigame2.tscn",
			[0, 0],
...
## Called when the Mooneten Storage button is pressed. Checks if the player has enough Moonstone
...
func _on_Moonetenstorage_button_pressed():
	var building_data: Array = DataScript.shop_data
	if DataScript.get_moonstone() >= building_data[DataScript.MOONETEN_STORAGE_CARD]["price"]:
		DataScript.set_inventory(
...
## Called when the Moonstone Generator button is pressed. Checks if the player has enough Mooneten
...
func _on_Moonstonegenerator_button_pressed():
	var building_data: Array = DataScript.shop_data
	if DataScript.get_mooneten() >= building_data[DataScript.MOONSTONE_GENERATOR_CARD]["price"]:
		DataScript.set_inventory(
...
## Called when the Moonstone Storage button is pressed. Checks if the player has enough Mooneten
...
func _on_Moonstonestorage_button_pressed():
	var building_data: Array = DataScript.shop_data
	if DataScript.get_mooneten() >= building_data[DataScript.MOONSTONE_STORAGE_CARD]["price"]:
		DataScript.set_inventory(
...
## Called when the close button is pressed. Hides the building card and makes HUD visible.
...
# Function: set_shop
## Sets up the shop by resetting it, populating it with building data, and updating resource amounts.
func set_shop():
	reset_shop()

	building_card.visible = true
	var building_data: Array = DataScript.shop_data.duplicate(true)
...
# Function: reset_shop
## Removes all child nodes from the building_card parent.
func reset_shop():
...
## Sets up the building data in the shop. Sets the building name, button, price, count, image, needed resources,
...
## Sets the visibility of Mooneten and Moonstone images based on the needs of the building.
...
## Sets the text for the building_count label based on the building_id and whether the building is bought.
...
## Sets the price label for a building in the shop. Adjusts the label's text and custom_minimum_size based on the length of the price.
...
## Sets the image for a building in the shop based on its building_id.
...
## Sets up a building button in the shop. Creates a duplicate of the buy_button, connects its pressed signal
...
## Sets the "is_bought" attribute of the building in the shop_data at the given card_index to true.
...
## Checks if the player has enough Mooneten to purchase a building with the given price. [br]
...
## Removes the Moonenten cost of the building at the given card_index from the player's resources.
...
## Removes the Moonstone cost of the building at the given card_index from the player's resources.
...
## Disables the given button and sets the building card's modulation to DIM_GRAY.
...
## Sets the building card's modulation back to WHITE.
func set_card_enabled():
	building_card.modulate = Color.WHITE


# Function: needed_ressource_check
## Checks the building type and sets the appropriate resource requirements.
